			...
			{filteredFeatured.length > 0 && (
				<section id="one-cikan-projeler" className="space-y-6">
					...
					<Suspense fallback={<LoadingState label="Projeler yükleniyor..." variant="section" />}>
						<div className={resolveGridClass(filteredFeatured.length)}>
							{filteredFeatured.map((p, index) => (
								<RevealOnScroll key={p.title} animation="fade-up" delay={100 + (index * 100)}>
									<div className="group relative">
										...
										<div className="relative overflow-hidden rounded-xl sm:rounded-2xl border border-blue-200/40 dark:border-blue-700/40 bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm shadow-sm transition-all duration-500 group-hover:-translate-y-1 group-hover:shadow-lg">
											<ProjectCard {...p} />
										</div>
									</div>
								</RevealOnScroll>
